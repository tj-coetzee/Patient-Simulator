...
""" old loading stuff i am taking out
beginning = 0
...
def update_progress():
    ...
    loading_window.destroy()  # Close the loading window when done
    beginning = 1
    #create_main_window()  # Once the loop is done, calls the function to show the main application window

# Create a new Tkinter window for loading
loading_window = tk.Tk()
loading_window.title('Loading')  # title of the window
loading_window.geometry('1200x600')  # size of the window
...
def create_profile_manager_window():
    ...
    #profile_manager_window.mainloop()

"""def create_connection_window():
    # Create a new top-level window for connection status
    connection_window = tk.Toplevel()
    connection_window.title('Connection')  # Set the window title
    connection_window.geometry('1200x600')  # Set the window size
    ...
    # Create a 'Back' button to close the connection window
...
def create_connection_window():
    ...
    def connect_and_transfer():
        ...
        try:
            ...
            for item in os.listdir(os.getcwd()):
                if os.path.isdir(os.path.join(os.getcwd(), item)) and item.startswith("PROF_"):
                    if item not in existing_directories:  # Only transfer if directory does not exist
                        # If the item is a directory starting with 'PROF_', transfer it
                        # recursive=True makes sure it includes all the subdirectories and files contained
                        scp.put(os.path.join(os.getcwd(), item), recursive=True, remote_path='/home/pi/')
                    else:
                        print(f"Skipping {item}, already exists.")

            # Display a success message upon successful transfer
...
def wave_plot_ECG(profile_name):
    ...
    # Only works for header files named header:
    ...
    if os.path.exists(header_file_path):
        ...
        # This option only works for data files named data.dat
...
main_window.geometry('1200x600')  # Set the dimensions of the window
