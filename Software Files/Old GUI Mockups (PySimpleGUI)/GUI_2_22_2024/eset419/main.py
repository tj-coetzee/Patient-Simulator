...
# main window first visit
main_first_visit = True
...
while True:
    ...
    for prof_name in os.listdir(os.getcwd()):
        if prof_name.startswith('PROF_') and os.path.isdir(prof_name):
            ...
            # Draw picture the first time the "Plot" button is pressed            
            if main_first_visit:
                plt = wave_plot(profile_name)
                draw_figure(main_window['-CANVAS-'].TKCanvas, plt)
                main_first_visit = False
            elif main_event == f'-PLOT-{profile_name}-':
                clear_figures(main_window['-CANVAS-'].TKCanvas)
